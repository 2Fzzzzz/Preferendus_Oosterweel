...
w1 = 0.3   #City of Antwerp
w2 = 0.1   #Inhabitants
w3 = 0.2   #Lantis(Project manager)
w4 = 0.1   #Enviromental Group
w5 = 0.3   #Contractor
...
X_POINTS_CAPACITY, P_POINTS_CAPACITY = [[7337, 9500, 12692], [0, 50, 100]]       #Capacity
...
b2 = [4, 6]             #Lanes X2
b3 = [5.5, 6.2]         #Inner Height(m) X3
b4 = [1.0, 1.6]         #Thickness(m) X4
...
def calculate_construction_time(x1, x2, x3, x4, x5, x6, x8, x9):
    denom = np.maximum(x8 * x9, 1e-6)  # prevent division by zero
...
def constraint_min_thickness(variables):
    x3 = variables[:, 2]  # inner height
    x4 = variables[:, 3]  # wall thickness
    return 0.12 * x3 - x4  # ensures x4 ≥ 0.12 * x3

def constraint_max_thickness(variables):
    x3 = variables[:, 2]
    x4 = variables[:, 3]
    return x4 - 0.25 * x3  # ensures x4 ≤ 0.25 * x3

def constraint_aspect_ratio_min(variables):
    x2 = variables[:, 1]  # lanes
    x3 = variables[:, 2]  # inner height
    x5 = variables[:, 4]  # lane width
    return 2.5 - (x2 * x5 / x3)  # ensures width/height ≥ 2.5

def constraint_aspect_ratio_max(variables):
    x2 = variables[:, 1]
    x3 = variables[:, 2]
    x5 = variables[:, 4]
    return (x2 * x5 / x3) - 5.5  # ensures width/height ≤ 5.5

def constraint_speed_width(variables):
    x5 = variables[:, 4]  # lane width
    x6 = variables[:, 5]  # speed limit
    # empirical: speed ≈ 15*width + 30 ± 10
    return x6 - (15 * x5 + 30)  # ensures x6 ≤ 15x5 + 30

def constraint_min_height(variables):
    x3 = variables[:, 2]
    return 5.5 - x3  # ensures min inner clearance for trucks
...
#cons = []
cons = [['ineq', constraint_min_thickness], ['ineq', constraint_max_thickness], ['ineq', constraint_aspect_ratio_min], ['ineq', constraint_aspect_ratio_max]
        , ['ineq', constraint_speed_width], ['ineq', constraint_min_height], ['ineq', constraint_construction_equipment]]
